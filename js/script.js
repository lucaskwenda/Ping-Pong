...
let telaInicial = document.querySelector('.comecarJogo')
...
function colisao(){
//Colisão com o jog1
if((posBolaX <= posJ1x+20)&&((posBolaY+20 >= posJ1y)&&(posBolaY <= posJ1y+180))
){
    dirBolaX=1;
    clearInterval(jogo);
    if(frames > 8){
...
//Colisão com o jog2
if((posBolaX >= posJ2x-20)&&((posBolaY+20 >= posJ2y)&&(posBolaY <= posJ2y+180))){
    dirBolaX=-1;
    clearInterval(jogo);
    if(frames > 8){
    frames-=2;
...
function controlajog(){
    if(dirBolaX == -1){
        ...
        document.getElementById("jog1").style.backgroundColor="yellow";
        ...
        if((posJ1y+180) >= campoH+40){
            ...
            document.getElementById("jog2").style.backgroundColor="yellow";
...
function reset(){
    ...
    setTimeout(()=>{
        ...
        document.getElementsByTagName('h1')[0].innerHTML = "Gooooolll"
    },100)
